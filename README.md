...
It features an intelligent detection system that automatically adapts the installation flow based on your inventory, supporting both:
...
Used when all components are assigned to the same node.

- A confirmation prompt is required (`YES`)
- A warning about scalability limitations is displayed
- The `single_server_setup` role is executed automatically at the end

#### Limitations

The following groups are **not supported** and must be empty:
...
```bash
ansible-playbook -i your_inventory_file zxbot.carbonio_install.carbonio_install
```
